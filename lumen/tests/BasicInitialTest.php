...
class BasicInitialTest extends TestCase
{
    ...
    public function SitemapCategoryAddTest()
    {
        ...
        $response = $this->call('POST', 'http://localhost:5000/sitemap_categories/'.urlencode($tmp_name));
    ...
    public function MerchantAddTest($sitemap_category_id) {
        ...
        $error_response = $this->call('POST', 'http://localhost:5000/merchants/'.urlencode($tmp_name).'/'.urlencode($tmp_url).'/'.urlencode($tmp_description).'/1');
        ...
        $success_response = $this->call('POST', 'http://localhost:5000/merchants/'.urlencode($tmp_name).'/'.urlencode($tmp_url).'/'.urlencode($tmp_description).'/'.$sitemap_category_id);
        ...
        $error_response = $this->call('POST', 'http://localhost:5000/merchants/'.urlencode($tmp_name).'/'.urlencode($tmp_url).'/'.urlencode($tmp_description).'/'.$sitemap_category_id);
        $this->assertEquals(412, $error_response->status());
    ...
    public function SitemapCategoryRemoveTest($sitemap_category_id, $merchant_id)
    {
        ...
        $error_response = $this->call('DELETE', 'http://localhost:5000/sitemap_categories/'.$sitemap_category_id);
        $this->assertEquals(412, $error_response->status());
        
        // then let's remove the merchant
        $error_response = $this->call('DELETE', 'http://localhost:5000/merchants/'.$merchant_id);
